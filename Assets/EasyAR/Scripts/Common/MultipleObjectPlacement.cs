...
public class MultipleObjectPlacement : MonoBehaviourPunCallbacks
{
    /// <summary>
    ...
    public void spawnObject(GameObject go)
    {
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            spawnedObject = PhotonNetwork.Instantiate(go.name, Vector3.zero, Quaternion.identity);
    ...
    public void PutBox(GameObject go)
    {
        foreach (GameObject obj in Object.FindObjectsOfType(typeof(GameObject)))
        {
            if (obj.name == "Pointer(Clone)")
            {
